<template>
  <div class="container">
    ...
    <section>
      ...
      <div>
        <label>pth 文件:</label>
        <input type="file" accept=".pth" @change="e => pth = e.target.files[0]?.name || ''" />
      </div>
      <div>
        <label>index 文件:</label>
        <input type="file" accept=".index" @change="e => index = e.target.files[0]?.name || ''" />
      </div>
    </section>
    ...
    <section>
      ...
      <div>
        ...
        <select v-model="hostapi">
          <option v-for="h in hostapis" :key="h" :value="h">{{ h }}</option>
        </select>
        <label><input type="checkbox" v-model="wasapiExclusive" /> 独占 WASAPI 设备</label>
      </div>
      <div>
        <label>输入设备:</label>
      ...
      <div>
        <button type="button" @click="reloadDevices">重载设备列表</button>
        <label><input type="radio" value="sr_model" v-model="srType" /> 使用模型采样率</label>
        <label><input type="radio" value="sr_device" v-model="srType" /> 使用设备采样率</label>
      </div>
    </section>
    ...
    <section>
      ...
      <div class="slider">
        ...
        <input type="range" min="-60" max="0" v-model.number="threshold" />
      </div>
      <div class="slider">
        <label>音调设置 {{ pitch }}</label>
        <input type="range" min="-16" max="16" v-model.number="pitch" />
      </div>
      <div class="slider">
        <label>性别因子 {{ formant }}</label>
        <input type="range" min="-2" max="2" step="0.05" v-model.number="formant" />
      </div>
    </section>
    ...
    <section>
      ...
      <div class="slider">
        ...
        <input type="range" min="0.02" max="1.5" step="0.01" v-model.number="blockTime" />
      </div>
      <div class="slider">
        <label>淡入淡出长度 {{ crossfadeLength }}</label>
        <input type="range" min="0.01" max="0.15" step="0.01" v-model.number="crossfadeLength" />
      </div>
    </section>
